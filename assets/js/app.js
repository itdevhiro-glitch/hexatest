...
export function renderTalents(target='#talentGrid'){
 ...
 wrap.innerHTML=TALENTS.map(t=>`<a class="talent-card" href="profile.html?id=${t.slug}" aria-label="Open profile ${t.name}"><img src="${talentImage(t.slug)}" alt="${t.name}"><div class="talent-info"><span class="eyebrow">${t.debut}</span><h3>${t.icon} ${t.name}</h3><p class="muted">${t.title}</p><div class="tags">${t.tags.map(x=>`<span class="tag">${x}</span>`).join('')}</div></div></a>`).join('');
}
...
export function initProfile(){
 ...
 $('#profileRoot').innerHTML=`<section class="profile-hero container"><div class="profile-art"><img src="${talentImage(t.slug)}" alt="${t.name}"></div><div class="profile-copy"><span class="eyebrow">${t.debut}</span><h1>${t.icon} ${t.name}</h1><p class="muted">${t.quote}</p><p>${t.bio}</p><div class="profile-stats"><div class="stat"><b>Constellation</b><br><span class="muted">${t.constellation}</span></div><div class="stat"><b>Role</b><br><span class="muted">${t.title}</span></div><div class="stat"><b>Schedule</b><br><span class="muted">${t.schedule}</span></div></div><div class="tags">${t.tags.map(x=>`<span class="tag">${x}</span>`).join('')}</div><div class="actions"><a class="btn primary" href="${t.social.youtube}">YouTube</a><a class="btn" href="${t.social.twitter}">X/Twitter</a><a class="btn" href="${t.social.discord}">Discord</a></div></div></section>`;
}
